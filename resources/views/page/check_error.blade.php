...
@section('content')
    ...
    <div class="container">
        ...
        @if(isset($isData) )
            ...
            {{-- List 1--}}
            @include('component.table_error',
            ['title' => '01-บันทึก เวลาที่เกิดเหตุ (Atime) และ เวลาที่มาถึง รพ. (Htime) ทุกราย',
            'list' => $is_List1])
            <hr>
            @include('component.table_error',
            ['title' => '02-อุบัติเหตุขนส่ง  การบาดเจ็บ (Injby) ต้องไม่มีรหัส  2 - ทำร้ายตนเอง  3 – ผู้อื่นทำร้าย',
            'list' => $is_List2])
            <hr>
            @include('component.table_error',
            ['title' => '03-เด็กอายุ < 5 ปี ไม่ควรทำร้ายตนเอง (Injby = 2)',
            'list' => $is_List3])
            <hr>
            @include('component.table_error',
            ['title' => '04-* บันทึกพฤติกรรมเสี่ยงโทรศัพท์มือถือ (Risk 5)ทุกราย',
            'list' => $is_List4])
            <hr>
            @include('component.table_error',
            ['title' => '05-* บันทึก ICDcause ทุกรายยกเว้น อุบัติเหตุขนส่งที่ไม่ต้องบันทึกเพราะใช้ช่อง(cause =1)แทน',
            'list' => $is_List5])
            <hr>
            @include('component.table_error',
            ['title' => '06-* บันทึกการมาจากที่เกิดเหตุทุกราย( Atohosp)โดยถ้าเป็นการมาด้วย หน่วยบริการการแพทย์ฉุกเฉิน (EMS)ให้ระบุระดับ ALS,BLS,หรือFR',
            'list' => $is_List6])

            <hr>
            @include('component.table_error',
            ['title' => '07-เด็กอายุต่ำกว่า 5 ปี ไม่ควรดื่มแอลกอฮอล์ ( Risk1<>0)',
            'list' => $is_List7])
            <hr>
            @include('component.table_error',
            ['title' => '08-ไม่ควรมีบาดเจ็บในอาชีพจากการทำร้ายตนเอง  (Injby = 2)',
            'list' => $is_List8])

            <hr>
            @include('component.table_error',
            ['title' => '09-* DBA ไม่ควรมาโรงพยาบาลโดยไม่มีผู้นำส่ง ( Atohosp =0 )',
            'list' => $is_List9])
            <hr>
            @include('component.table_error',
            ['title' => '10-อุบัติเหตุตกน้ำ จมน้ำ(ICDcause=W65-W74) จุดเกิดเหตุไม่ควรเป็นถนน ( Apoint = 5)',
            'list' => $is_List10])
            <hr>

            @include('component.table_error',
            ['title' => '11-บาดเจ็บจากอุบัติเหตุ (Injby = 1) แต่สาเหตุเป็นเจตนา icdcause = X60-Y09',
            'list' => $is_List11])
            <hr>
            ...
            <hr>
            @include('component.table_error',
            ['title' => '12-บาดเจ็บจากการทำร้ายตนเอง (Injby = 2) แต่สาเหตุไม่ทำร้ายตนเองด้วยวิธีต่าง ๆ  ( ICD cause<> X60-X84)',
            'list' => $is_List12])
            <hr>
            @include('component.table_error',
            ['title' => '13-บาดเจ็บจากผู้อื่นทำร้าย (Injby =3) แต่สาเหตุไม่ถูกทำร้ายด้วยวิธีต่าง ๆ (ICDcause =X85-Y09)',
            'list' => $is_List13])
            <hr>
            @include('component.table_error',
            ['title' => '14-* ถ้าบาดเจ็บจากอาชีพ (injoccu)ต้องมีการบันทึกอาชีพ (occu) ทุกราย',
            'list' => $is_List14])
            <hr>
            @include('component.table_error',
            ['title' => '15-ผู้บาดเจ็บ ที่มี  br = 6  ais = 1 ไม่สมควรตาย หากตายควรมีเหตุผลแนบ จำหน่ายผู้ป่วยออกจาก er (staer=1 – เสียชีวิตก่อนถึง รพ., 6-ถึงแก่กรรม) จำหน่ายผู้ป่วยจากward (staward=5 ถึงแก่กรรม) (ใช้กับ is ของ รพท/รพศ)',
            'list' => $is_List15])
            <hr>
            @include('component.table_error',
           ['title' => '16-* ต้องมีข้อมูลเพศทุกคน (sex= 1 ชาย  2- หญิง)',
           'list' => $is_List16])
            <hr>
            @include('component.table_error',
             ['title' => '17-bp, rr, p ไม่ควรเกินกว่ากำหนด (bp1 ไม่เกิน 300 bp2 ไม่เกิน 300 pr ไม่เกิน 200 rr ไม่เกิน 60 )(ใช้กับ is ของ   รพท/รพศ)',
             'list' => $is_List17])
            <hr>
            @include('component.table_error',
            ['title' => '18-จำหน่ายผู้ป่วยออกแล้ว (staward) แต่ไม่มีวันที่จำหน่าย (rdate)',
            'list' => $is_List18])
            <hr>
            @include('component.table_error',
            ['title' => '19-* ต้องมีข้อมูลอายุเป็น ปี (age) หรือเดือน (month) หรือวัน (year) ทุกคน',
            'list' => $is_List19])
            <hr>
            @include('component.table_error',
            ['title' => '20-ถ้าผู้ป่วยมีการ admit ทุกรายต้องแสดงสถานภาพการจำหน่ายจาก  ward ( หาก staer=7 <รับไว้รักษา> การ staward ต้องมีสถานภาพการจำหน่ายอย่างใดอย่างหนึ่ง)',
            'list' => $is_List20])
            <hr>
            @include('component.table_error',
            ['title' => '21-* ต้องมีการบันทึกสถานที่เกิดเหตุ (aplace)ทุกรายให้ครบถึงหมู่บ้าน',
            'list' => $is_List21])
            <hr>
            @include('component.table_error',
            ['title' => '22-* บันทึกจุดเกิดเหตุทุกราย ( apoint)',
            'list' => $is_List22])
            <hr>
            @include('component.table_error',
            ['title' => '24-* บันทึกพฤติกรรมเสี่ยงแอลกอฮอล์ (risk 1)',
            'list' => $is_List24])
            <hr>
            @include('component.table_error',
            ['title' => '26-* บันทึกพฤติกรรมเสี่ยงเข็มขัดนิรภัยทุกราย (risk 3) ในกรณีอุบัติเหตุขนส่ง (cause=1) ของรถยนต์ทุกชนิด (v40-79)',
            'list' => $is_List26])
            <hr>
            @include('component.table_error',
            ['title' => '27-* บันทึกพฤติกรรมเสี่ยงหมวกนิรภัยทุกราย (risk 4) ในกรณีอุบัติเหตุขนส่ง (cause=1) ของรถจักรยานยนต์ทุกชนิด (v20-29)',
            'list' => $is_List27])
