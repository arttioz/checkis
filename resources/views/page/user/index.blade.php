                ...
                <table class="table table-main table-bordered" id="dynamic-table">
                    ...
                    @foreach($users as $item)
                        <tr>
                            ...
                            <td> {{ $item->username ?? "" }}   </td>
                            <td> {{ $item->email ?? "" }}   </td>
                            <td> {{ $item->type=='1' ? "กองป้องกันการบาดเจ็บ" : "" }}
                                {{ $item->type=='2' ? "สคร." : "" }}
                                {{ $item->type=='3' ? "สสจ." : "" }}
                                {{ $item->type=='4' ? "โรงพยาบาล" : "" }}  {{ $item->hospcode ?? "" }}
                            </td>
                            <td>  {{ $item->type ?? "" }} </td>
                            <td class="center">    @if($item->status=='1')
                                    <span class="label label-success">เปิดใช้งาน</span>
                                @elseif($item->status=='0')
                                    <span class="label label-inverse arrowed-in">ระงับการใช้งาน</span>
                                         @endif
                            </td>
                            <td class="center">
                                <a href="{{ route('user.edit',$item->id)}}" class="btn btn-white btn-info btn-bold   "><i class="ace-icon fa fa-pencil  blue"></i> Edit</a>
                            </td>
